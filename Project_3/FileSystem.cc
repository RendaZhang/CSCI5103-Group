...
/**
 * @brief Get the MaxBlockNumber for the single indrect block structure
 * @return the number of the max block number
 */
int FileSystem::getMaxBlockNumber()
{
	return IndexNode::MAX_DIRECT_BLOCKS + blockSize / sizeof(int);
}

/**
 * @brief scan all the blocks and check if an allocated block is not in inodes
 */
void FileSystem::scan(map<int,int> &allocated)
{
	for(int i = 0; i < blockCount; i++)
	{
		if(!is_free(i) && allocated.find(i) == allocated.end())
		{
			cout << "fsck detect error: allocated block " << i
			<< " is not in inodes" << endl;
		}
	}
}
 ...
 */
bool FileSystem::is_free(int block) {
	loadFreeListBlock(block);
	return (!(freeListBitBlock->isBitSet(block % (blockSize * 8))));
}
...
int FileSystem::getCurrentIndexNodeNumber() {
