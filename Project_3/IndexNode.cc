...
/**
 * @breif get all the blocks
 */
void IndexNode::getBlocks(stack<int> &s)
{
...
int IndexNode::getBlockAddress(int block)
{
	...
	else if(block < fileSystem->getMaxBlockNumber()) {
    int blockSize = fileSystem->getBlockSize();
    char blocks[blockSize];
    // Get the address of the current indirectBlock
    fileSystem->read(blocks, fileSystem->getDataBlockOffset() + indirectBlock);
		return ((int*)blocks)[block];
	}
	else {
...
void IndexNode::setBlockAddress(int block, int address)
{
	...
	if(block < fileSystem->getMaxBlockNumber()) {
	  int blockSize = fileSystem->getBlockSize();
		// multiple of 4 for block size
		int size = blockSize / sizeof(int);
		int blocks[size];

		if(indirectBlock == NOT_A_BLOCK){
			cout << endl;
			cout << "Initialize indrect block" << endl;
			cout << "Setting indirect block" << endl;
			cout << endl;
			// allocate blocks for the indirectBlock
			...
			// Initialize all the blocks in indirectBlock
			for(int i = 0; i < size; i++)
			{
				blocks[i] = NOT_A_BLOCK;
			}
		}
		else
		{
			cout << endl;
			cout << "Setting indrect block" << endl;
			cout << endl;
			// Get the current indirectBlock
	   	fileSystem->read((char*)blocks, fileSystem->getDataBlockOffset() + indirectBlock);
		}

	  blocks[block] = address;
	  fileSystem->write((char*)blocks, fileSystem->getDataBlockOffset() + indirectBlock);
	}
	else
	{
		cout << "Invalid block" << endl;
