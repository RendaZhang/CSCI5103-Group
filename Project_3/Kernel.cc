...
int Kernel::unlink(char *pathname) {
  ...
  char name[512];
  memset(name, '\0', 512);
  // find the directory name of the file
  while (1) {
    if(token != NULL) {
      memset(name, '\0', 512);
      strcpy(name, token);
  ...
  // Scan the directory entry
  int status = 0;
  DirectoryEntry currentDirectoryEntry;
  while(true) {
    // Read Directory entry from the directory
    ...
    if(status < 0) {
      ...
      if(!strcmp(currentDirectoryEntry.getName(), name)) {
        // break the loop when it pass the directory entry
        break;
      }
    }
  }
  ...
  // Shift all he directory entries for deletion
  while(true) {
    status = readdir(dir, currentDirectoryEntry);
    if(status < 0) {
      cout << PROGRAM_NAME << ": error reading directory in creat";
      ...
      // Seek forward past the one that was written
  ...
  // Check if nlinks is 0 and if so then free the blocks
  int nlinks = inode.getNlink();
  if(nlinks > 0) {
    inode.setNlink(nlinks);
    fileSystem->writeIndexNode(&inode, inodeNumber);
  }
  else {
    // free all blocks that is allocated to the file
...
int Kernel::link(char *oldpath, char *newpath) {
  ...
  IndexNode tempNode;
  short newInodeNumber = findIndexNode(newFullPath, tempNode);
  // get full path again for unknown reason
  newFullPath = getFullPath(newpath);

  char dirname[1024];
  memset(dirname, '\0', 1024);
  strcpy(dirname, "/");
  char *token = NULL;
  token = strtok(newFullPath, "/");
  char name[512];
  memset(name, '\0', 512);
  // find the directory name of the file
  while(1) {
  ...
  // new path doesn't exit
  if(newInodeNumber < 0) {
    sourceIndexNode.setNlink(sourceIndexNode.getNlink() + 1);
    ...
    while(true) {
      ...
      // If no entry read, write the new directory entry
      else if( status == 0 ) {
        writedir(dir , newDirectoryEntry);

        // Update the fileSystem
        IndexNode temp;
        fileSystem->writeIndexNode(&temp, sourceInodeNumber);

        break;
      }
      // when there is driectory entry in the current diretory
      else {
        if(strcmp(currentDirectoryEntry.getName(),newDirectoryEntry.getName()) > 0) {
          ...
          // write the new item in place of the old one and break
          writedir(dir, newDirectoryEntry);
          break ;
        }
      }
    ...
    // check for more direcoty entry and copy the rest of the directory entries
    DirectoryEntry nextDirectoryEntry;
    while(status>0)	{
      // read next diretory entry
...
// Scan for error in filesystem
int Kernel::fsck()
{
	std::stack<int> Stack;
  Stack.push(FileSystem::ROOT_INDEX_NODE_NUMBER);
	std::map<int, int> expectedLinkCounts;
	std:map<int, int> Blocks;
	std::map<int, int> nlinkCounts;
	nlinkCounts[FileSystem::ROOT_INDEX_NODE_NUMBER] = 0;

	while(!Stack.empty())
  {
		int inode_stack_num  = Stack.top();
		Stack.pop();

		IndexNode inode;
		openFileSystems->readIndexNode(&inode, inode_stack_num);

		expectedLinkCounts[inode_stack_num] = inode.getNlink();
		...
		while(!blocks.empty())
    {
      ...
      blocks.pop();
			Blocks[addr] = 1;
			if(openFileSystems->is_free(addr))
      {
        cout << "fsck detect error: free block " << addr
        << " is in the inode " << inode_stack_num << endl;
			}
		}
		...
		if ((inode.getMode() & S_IFMT) == S_IFDIR)
    {
			int fd = open(new FileDescriptor(openFileSystems, inode, O_RDONLY));
			if (fd < 0)
      {
				cout << "failed to open node " << inode_stack_num << endl;
			...
			for(int status = readdir(fd, dirEntry); status > 0; status = readdir(fd, dirEntry))
      {
				int nextInodeNumber = dirEntry.getIno();
				if(nlinkCounts.find(nextInodeNumber) == nlinkCounts.end())
        {
					Stack.push(nextInodeNumber);
					nlinkCounts[nextInodeNumber] = 0;
				}
					nlinkCounts[nextInodeNumber] += 1;
			}
			close(fd);
		}
	}

  // increment 1 to match for link count result in mkdir.cc
  nlinkCounts[0] += 1;

	// Verify that the nlinks counts are as expected.
	for (map<int, int>::iterator iter = nlinkCounts.begin(); iter != nlinkCounts.end(); iter++)
  {
		int inodeNumber = iter->first;
		if(expectedLinkCounts[inodeNumber] != nlinkCounts[inodeNumber])
    {
			cout << "fsck detect error: inode " << inodeNumber << " has incorrect nlinks: "
      << nlinkCounts[inodeNumber] << "; The expected nlinks is: "
      << expectedLinkCounts[inodeNumber] << endl;
		}
	}

	openFileSystems->scan(Blocks);

  cout << "fsck finish scanning." << endl;
...
/**
 * @brief break the filesystem for test purpose
 */
int Kernel::BreakFilesystem()
{
  IndexNode root;
  openFileSystems->readIndexNode(&root, FileSystem::ROOT_INDEX_NODE_NUMBER);
  root.setNlink(2 + root.getNlink());
...
void Kernel::defrag() {
  ...
  int owner[blocks];
  int address[blocks];
  int ownerOrder[blocks];
  int addressOrder[blocks];
  int curOrderPos = 0;
  int maxOrderPos = 0;
  ...
  // find all blocks
  for(int inum=0; inum<40; inum++){
    IndexNode inode;
    fs->readIndexNode(&inode, inum);
    int size = inode.getSize();
    int numBlocks = (size + fs->getBlockSize()-1) / fs->getBlockSize();
    if(fs->is_free(inode.getBlockAddress(0))) {
      numBlocks = 0;
      inode.setSize(0);
      fs->writeIndexNode(&inode, inum);
    }
    // get the address of each block
    for(int j=0; j<numBlocks; j++){
      int addr = inode.getBlockAddress(j);
      owner[addr] = inum;
      address[addr] = j;
      ownerOrder[curOrderPos] = inum;
      addressOrder[curOrderPos] = j;
      curOrderPos++;
    }
  }

  maxOrderPos = curOrderPos;
  curOrderPos = 0;
  int curBlock = 0;

  while(curBlock < blocks && curOrderPos < maxOrderPos) {
    char temp[fs->getBlockSize()];
    char temp2[fs->getBlockSize()];
    IndexNode inode;
    IndexNode inode2;
    if(fs->is_free(curBlock)){
      // move
      fs->allocateBlock();
      fs->readIndexNode(&inode, ownerOrder[curOrderPos]);
      int oldBlock = inode.getBlockAddress(addressOrder[curOrderPos]);
      fs->read(temp, oldBlock);
      fs->write(temp, curBlock);
      inode.setBlockAddress(addressOrder[curOrderPos], curBlock);
      fs->writeIndexNode(&inode, ownerOrder[curOrderPos]);
      fs->freeBlock(oldBlock);
      curOrderPos++;
    } else if(owner[curBlock] != -1) {
      // swap
      // modify first inode
      fs->readIndexNode(&inode, ownerOrder[curOrderPos]);
      int oldBlock = inode.getBlockAddress(addressOrder[curOrderPos]);
      ...
      // modify second inode
      fs->readIndexNode(&inode2, owner[curBlock]);
      inode2.setBlockAddress(address[curBlock], oldBlock);
      fs->writeIndexNode(&inode2, owner[curBlock]);

      fs->read(temp, oldBlock);
      fs->read(temp2, curBlock);
      fs->write(temp, curBlock);
      fs->write(temp2, oldBlock);
      curOrderPos++;
    }
    curBlock++;
  }
